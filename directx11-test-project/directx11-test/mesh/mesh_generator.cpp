...
xtest::mesh::MeshData xtest::mesh::GenerateTorus(float r, float R, int nsides, int rings)
{
	MeshData mesh;
	int i, j;
	...
	for (i = 0; i <= rings; i++)
	{
		phi = (float)i * 2.0f * XM_PI / rings;
		phi1 = (float)(i + 1) * 2.0f * XM_PI / rings;
		for (j = 0; j <= nsides; j++)
		{
			theta = (float)j * 2.0f * XM_PI / nsides;
			theta1 = (float)(j + 1) * 2.0f * XM_PI / nsides;
			MeshData::Vertex vertex1;
			vertex1.position.x = cosf(phi) * (R + r * cosf(theta));   // x
			vertex1.position.y = sinf(phi) * (R + r * cosf(theta));  // y
			vertex1.position.z = r * sinf(theta);                     // z

			XMVECTOR position1 = XMLoadFloat3(&vertex1.position);
			XMStoreFloat3(&vertex1.normal, XMVector3Normalize(position1));

			mesh.vertices.push_back(vertex1);
			MeshData::Vertex vertex2;
			vertex2.position.x = cosf(phi) * (R + r * cosf(theta1));   // x
			vertex2.position.y = sinf(phi) * (R + r * cosf(theta1));  // y
			vertex2.position.z = r * sinf(theta1);                     // z

			XMVECTOR position2 = XMLoadFloat3(&vertex2.position);
			XMStoreFloat3(&vertex2.normal, XMVector3Normalize(position2));

			mesh.vertices.push_back(vertex2);
			MeshData::Vertex vertex3;
			vertex3.position.x = cosf(phi1) * (R + r * cosf(theta1));   // x
			vertex3.position.y = sinf(phi1) * (R + r * cosf(theta1));  // y
			vertex3.position.z = r * sinf(theta1);                     // z

			XMVECTOR position3 = XMLoadFloat3(&vertex3.position);
			XMStoreFloat3(&vertex3.normal, XMVector3Normalize(position3));

			mesh.vertices.push_back(vertex3);
			MeshData::Vertex vertex4;
			vertex4.position.x = cosf(phi1) * (R + r * cosf(theta));   // x
			vertex4.position.y = sinf(phi1) * (R + r * cosf(theta));  // y
			vertex4.position.z = r * sinf(theta);                     // z

			XMVECTOR position4 = XMLoadFloat3(&vertex4.position);
			XMStoreFloat3(&vertex4.normal, XMVector3Normalize(position4));

			mesh.vertices.push_back(vertex4);
