...
void BoxDemoApp::InitMatrices()
{
	...
	XMMATRIX viewMatrix = XMMatrixLookAtLH(
		XMLoadFloat4(&eyePosition),
		XMLoadFloat4(&focusPosition),
...
void BoxDemoApp::BuildProjectionMatrix()
{
...
void BoxDemoApp::InitShaders()
{
	...
	XTEST_D3D_CHECK(m_d3dDevice->CreateInputLayout(inputLayoutDesc, ARRAYSIZE(inputLayoutDesc), vsByteCode.Data(), vsByteCode.ByteSize(), &m_inputLayout));
}
...
void BoxDemoApp::InitBuffers()
{
	...
	XTEST_D3D_CHECK(m_d3dDevice->CreateBuffer(&vertexBufferDesc, &vertexSubData, &m_vertexBuffer));
	...
	XTEST_D3D_CHECK(m_d3dDevice->CreateBuffer(&indexBufferDesc, &indexSubData, &m_indexBuffer));
	...
	cBufferDesc.ByteWidth = sizeof(PerObjectCB);
	...
	XTEST_D3D_CHECK(m_d3dDevice->CreateBuffer(&cBufferDesc, nullptr, &m_vsConstantBuffer));
}
...
{ 
	...
	XTEST_D3D_CHECK(m_d3dContext->Map(m_vsConstantBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedRes));

	PerObjectCB* constantBuffer = static_cast<PerObjectCB*>(mappedRes.pData);
	XMMATRIX wvpT = XMMatrixTranspose(wvp);
	XMStoreFloat4x4(&(constantBuffer->WVP), wvpT);
...
void BoxDemoApp::RenderScene()
{
	...
	m_d3dContext->ClearRenderTargetView(m_backBufferView.Get(), DirectX::Colors::Aqua);
